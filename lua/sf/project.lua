...
M.get_current_package_dir = function()
  ...
  if project and project["packageDirectories"] then
    for i, dir in pairs(project["packageDirectories"]) do
      if dir["default"] then
        local package_path = dir["path"]
        package_path = vim.fs.normalize(root .. package_path .. "/main/default")
...
M.set_current_package_dir = function(package_name)
  ...
  current_package_dir = vim.fs.normalize(root .. package_name)
  if current_package_dir:sub(-1) ~= "/" then
    current_package_dir = current_package_dir .. "/"
  end
end
